...
| `next-api` is the unhealthy container; `/api/next/health` is slow or returns 503 | Migrations are not `ready`. The endpoint no longer does write work — the artwork-trash purge it used to run on every probe moved to the worker (`artwork.trash_purge`) | Wait out the migration. If the probe is still slow with migrations `ready`, the database itself is not answering, so look at `postgres` rather than at the API |
