...
def health_check(context=None):
    return {
        "status": "available",
        "message": "Zavvi provider is ready. Configure optional fallback currency/timeout if needed.",
    }
...
def price_check(payload=None, context=None):
    ...
    if price is None:
        return {"status": "no_match", "error": "No usable Zavvi price found in the product page."}
